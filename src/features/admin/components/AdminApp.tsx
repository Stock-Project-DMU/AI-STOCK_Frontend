...
function Card({ children, className = "" }: { children: ReactNode; className?: string }) {
  return <section className={`rounded-2xl border border-white/8 bg-[#0b1726] shadow-[0_18px_55px_rgba(0,0,0,.18)] ${className}`}>{children}</section>;
}
...
function Login({ onLogin }: { onLogin: () => void }) {
  ...
  return <main className="market-theme auth-shell min-h-screen text-zinc-100">
    <header className="flex h-16 items-center justify-between border-b border-white/10 bg-[#06101d] px-6 lg:px-10"><img src="/Logo.png" alt="AI STOCK" className="h-11 w-auto" /><div className="flex items-center gap-3"><ThemeToggle /><span className="theme-accent-border theme-accent-soft theme-accent-text rounded-full border px-3 py-1.5 text-[10px] font-black tracking-wider">ADMIN TERMINAL</span></div></header>
...
function Shell({ section, setSection, logout, children }: { section: Section; setSection: (v: Section) => void; logout: () => void; children: ReactNode }) {
  ...
  return <div className="market-theme market-grid min-h-screen bg-[#050c16] text-zinc-100 lg:flex">
    <aside className="border-b border-white/8 bg-[#07111e] lg:fixed lg:inset-y-0 lg:w-[264px] lg:border-b-0 lg:border-r">
      ...
      <nav className="flex gap-2 overflow-x-auto px-3 pb-3 lg:block lg:space-y-2 lg:px-4 lg:pb-0">
        {nav.map(([key, icon, label]) => <button key={key} onClick={() => setSection(key)} className={`flex min-w-max items-center gap-3 rounded-xl px-4 py-3 text-sm font-semibold transition lg:w-full ${section === key ? "admin-button-primary" : "admin-button-secondary"}`}><span className="w-5 text-center text-lg">{icon}</span>{label}</button>)}
      </nav>
    ...
    <div className="min-w-0 flex-1 lg:ml-[264px]">
      <header className="sticky top-0 z-20 flex min-h-20 flex-wrap items-center gap-4 border-b border-white/8 bg-[#07111e]/95 px-5 py-4 backdrop-blur lg:px-8"><div className="min-w-0 flex-1"><h1 className="truncate text-lg font-bold">{title} <span className="ml-2 text-xs font-normal text-zinc-600">{subtitle}</span></h1></div><input aria-label="전체 검색" placeholder="검색어를 입력하세요" className="hidden h-10 w-52 rounded-lg border border-white/10 bg-[#0b1726] px-4 text-sm outline-none placeholder:text-zinc-600 focus:border-[var(--market-accent)] md:block" /><ThemeToggle /><span className="text-sm text-zinc-400">관리자님</span><button onClick={logout} className="admin-button-secondary rounded-lg px-3 py-2 text-xs font-semibold">로그아웃</button></header>
...
function Dashboard({ open, go }: { open: (detail: Detail) => void; go: (section: Section) => void }) {
  return <div className="space-y-6">
    ...
    <Card><div className="flex items-center justify-between border-b border-white/8 p-5"><h2 className="font-bold">캐시 충전 요청</h2><button onClick={() => go("accounts")} className="admin-link-button text-xs font-semibold">전체보기 →</button></div><Table head={["요청번호", "사용자", "요청 금액", "요청 시간", "상태", "관리"]}>{cashRequests.slice(0,3).map(r => <tr key={r.id}><Td>{r.id}</Td><Td>{r.user}</Td><Td strong>{r.amount}</Td><Td>{r.requested}</Td><Td><Status>{r.status}</Status></Td><Td><Action onClick={() => open({kind:"account", id:r.id})}>심사</Action></Td></tr>)}</Table></Card>
    <Card><div className="flex items-center justify-between border-b border-white/8 p-5"><h2 className="font-bold">최근 거래</h2><button onClick={() => go("transactions")} className="admin-link-button text-xs font-semibold">전체보기 →</button></div><Table head={["거래번호", "사용자", "구분", "종목", "거래금액", "상태"]}>{transactions.slice(0,4).map(r => <tr key={r.id} onClick={() => open({kind:"transaction", id:r.id})} className="cursor-pointer"><Td>{r.id}</Td><Td>{r.user}</Td><Td><span className={r.type === "매수" ? "text-red-400" : "text-blue-400"}>{r.type}</span></Td><Td strong>{r.stock}</Td><Td>{r.amount}</Td><Td><Status>{r.status}</Status></Td></tr>)}</Table></Card>
  </div>;
}
...
function Action({ children, onClick, danger = false }: { children: ReactNode; onClick?: () => void; danger?: boolean }) { return <button onClick={onClick} className={`rounded-md px-3 py-1.5 text-xs font-semibold ${danger ? "border border-red-500/40 bg-red-500/10 text-red-400 hover:bg-red-500/20" : "admin-button-secondary"}`}>{children}</button>; }
...
function DetailPage({ detail, back }: { detail: NonNullable<Detail>; back: () => void }) {
  ...
  return <div><Crumb text={`회원 관리 > ${u.id}`} back={back}/><h2 className="mb-5 text-xl font-bold">회원 상세 정보</h2><div className="grid gap-5 xl:grid-cols-2"><InfoCard title="기본 정보" items={[["회원번호",u.id],["이름",u.name],["이메일",u.email],["가입일",u.joined],["계정 상태",u.status]]}/><InfoCard title="자산 및 활동" items={[["보유 캐시",u.balance],["보유 종목","7종목"],["누적 거래","128건"],["최근 접속","2026.07.20 14:22"],["신고 이력","0건"]]}/></div><Card className="mt-5 p-6"><h3 className="font-bold">계정 관리</h3><p className="mt-2 text-sm text-zinc-500">정지된 계정은 로그인과 신규 거래가 제한됩니다.</p><div className="mt-5 flex gap-3">{u.status==="정지"?<button onClick={()=>setResult("activated")} className="admin-button-primary rounded-lg px-5 py-2.5 text-sm font-bold">계정 활성화</button>:<button onClick={()=>setResult("suspended")} className="rounded-lg border border-red-500/40 bg-red-500/10 px-5 py-2.5 text-sm font-semibold text-red-400 hover:bg-red-500/20">계정 정지</button>}</div></Card></div>;
}

function Crumb({text,back}:{text:string;back:()=>void}){return <div className="mb-5 flex flex-wrap items-center justify-between gap-3"><p className="text-xs text-zinc-600">{text}</p><button onClick={back} className="admin-button-secondary rounded-lg px-4 py-2 text-xs font-semibold">← 목록으로</button></div>}
