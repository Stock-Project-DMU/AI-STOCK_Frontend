...
function DetailPanel({ detail, back }: { detail: NonNullable<Detail>; back: () => void }) {
    ...
    return <section className="rounded-lg border border-hairline bg-white p-5">
        ...
        {trade && <><h2 className="text-lg font-bold">주문 #{trade.order.orderId}</h2><p className="mt-3">{trade.userName} · {trade.order.stockName} ({trade.order.stockCode})</p><p className="mt-3"><StatusBadge value={trade.order.orderType} /> {trade.order.quantity}주 · {won(trade.order.execPrice ?? trade.order.orderPrice)} · <StatusBadge value={trade.order.status} /></p><p className="mt-3">{trade.order.orderedAt}</p>{trade.order.status === "PENDING" && <><textarea value={reason} onChange={event => setReason(event.target.value)} placeholder="취소 사유" className="mt-4 w-full rounded border p-3" /><button disabled={busy || !reason.trim()} onClick={() => void mutate("cancel", { reason })} className="mt-3 rounded border border-red-500 px-4 py-2 text-red-500 disabled:opacity-50">미체결 주문 취소</button></>}</>}
        {user && <><h2 className="text-lg font-bold">{user.name} ({user.loginId})</h2><p className="mt-3">{user.email} · <StatusBadge value={user.status} /></p><p className="mt-3">가입일 {user.createdAt}</p><h3 className="mt-5 font-bold">보유 계좌</h3>{user.accounts.map(account => <p className="mt-2" key={account.accountId}>{account.accountName} · {account.accountNumber} · {won(account.balance)}</p>)}<p className="mt-3">보유 종목 {user.holdings.length}개 · 주문 {user.orders.length}건</p><UserStatusAction user={user} onChanged={back} /></>}
    </section>;
}
