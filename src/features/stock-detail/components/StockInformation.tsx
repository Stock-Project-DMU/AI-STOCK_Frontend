...
export default function StockInformation({ stockCode }: { stockCode: string }) {
    ...
    return <div className="mx-auto flex max-w-[1500px] flex-col gap-3 p-4 lg:flex-row"><nav className="flex gap-2 overflow-auto lg:w-48 lg:shrink-0 lg:flex-col">{tabs.map(([key, title]) => <button key={key} onClick={() => setTab(key)} className={`shrink-0 rounded p-3 text-left text-sm ${tab === key ? "bg-primary text-white" : "bg-canvas"}`}>{title}</button>)}</nav><section className="min-w-0 flex-1 rounded-lg border border-hairline bg-canvas p-5">
