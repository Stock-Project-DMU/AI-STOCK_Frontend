        ...
        <div className="grid min-w-0 gap-3 lg:grid-cols-[minmax(0,1fr)_220px]">
            <div className="min-w-0 space-y-3">
                <section className="overflow-hidden rounded-lg border border-hairline bg-canvas">
                    <div className="flex flex-wrap items-center justify-between gap-3 border-b border-hairline px-4 py-3">
                        <div>
                            <div className="flex items-center gap-2">
                                ...
                                <span className="rounded-pill bg-emerald-500/10 px-2 py-0.5 text-[10px] font-bold text-emerald-500">LIVE</span>
                            </div>
                            <div className="num mt-1.5 flex gap-4 text-[10px] text-muted">
                                ...
                                <span>종 <b className="text-ink">191,000</b></span>
                            </div>
                        </div>
                    ...
                    <div className="relative px-2 pb-2 pt-2">
                        <div className="num absolute right-3 top-3 z-10 rounded-xs bg-[var(--market-up)] px-2 py-1 text-[10px] font-bold text-white">191,000</div>
                ...
                <section className="overflow-hidden rounded-lg border border-hairline bg-canvas">
                    <div className="flex items-center justify-between border-b border-hairline px-4 py-3">
                        <div><h2 className="font-bold text-ink">실시간 체결</h2><p className="mt-1 text-[10px] text-muted">최근 체결 내역</p></div>
            ...
            <aside className="overflow-hidden rounded-lg border border-hairline bg-canvas">
                <div className="border-b border-hairline px-3 py-3">
                    <div className="flex justify-between"><strong className="text-ink">실시간 호가</strong><span className="text-[10px] text-muted">체결강도 <b className="text-up">58.45%</b></span></div>
                    <div className="mt-2 grid grid-cols-2 text-[10px] text-muted"><span>가격</span><span className="text-right">잔량</span></div>
                </div>
                <div className="py-1">
                    ...
                    <div className="my-1.5 border-y border-[rgba(207,32,47,0.2)] bg-[rgba(207,32,47,0.05)] px-3 py-2">
                        <div className="num flex items-end justify-between"><strong className="text-base text-up">191,000</strong><span className="text-xs font-bold text-up">▲ 1.76%</span></div>
                        <p className="num mt-1 text-[10px] text-muted">전일 대비 +3,303</p>
                    </div>
                    {QUOTES.slice(5).map((quote, index) => <QuoteRow key={quote[0]} quote={quote} index={index} side="buy" />)}
                </div>
                <dl className="num grid grid-cols-2 gap-y-2 border-t border-hairline px-3 py-3 text-[10px] text-muted"><dt>52주 최고</dt><dd className="text-right font-semibold text-up">205,000</dd><dt>52주 최저</dt><dd className="text-right font-semibold text-down">168,000</dd><dt>시가</dt><dd className="text-right text-ink">192,500</dd><dt>거래량</dt><dd className="text-right text-ink">15,234,567</dd></dl>
            </aside>
        </div>
    );
}
...
function ChartTooltip({ active, payload }: TooltipContentProps) {
    ...
    return (
        <div className="rounded-md border border-hairline bg-canvas px-3 py-2 text-[11px] shadow-[0_8px_24px_rgba(10,11,13,0.12)]">
...
function ChartMetric({ label, value, positive }: { label: string; value: string; positive?: boolean }) {
    return <dl className="border-r border-hairline px-4 py-2.5 last:border-0"><dt className="text-[10px] text-muted">{label}</dt><dd className={`num mt-1 text-xs font-bold ${positive ? "text-up" : "text-ink"}`}>{value}</dd></dl>;
}
...
function QuoteRow({ quote, index, side }: { quote: readonly [string, string, string]; index: number; side: "sell" | "buy" }) {
    ...
    return (
        <div className="num relative grid grid-cols-[1fr_auto] overflow-hidden px-3 py-1.5 text-[11px]">
            ...
            <span className="relative text-[10px] text-muted">{quote[2]}</span>
        </div>
    );
}
