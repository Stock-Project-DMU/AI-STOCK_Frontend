...
## AI-STOCK Dashboard Density Addendum

The sections above describe Coinbase's **marketing** site only (explicitly out of scope: order books, charts, order forms). AI-STOCK is a Korean 증권사 HTS-style app, not a marketing site, so a second density tier is defined here for reuse across the app:

### Density tiers
- **`editorial`** — auth/marketing screens (`welcome`, `login`, `signup`, `find-id`, `find-password`, `account-recovery`). Uses the spacing scale above fairly literally (generous card padding, `rounded.xl`/`rounded.2xl` cards, `body-md` 16px text).
- **`dashboard`** — data-dense trading screens (`home`, `stock-detail`, `my-page`, `admin`, `Sidebar`). Tighter, HTS-appropriate scale:
  - Row vertical padding: 6–8px (vs. editorial's 16px+)
  - Base body text: 13–14px (vs. 16px)
  - Card padding: 12–16px (vs. 32px)
  - Card radius: `rounded.lg`/`rounded.md` (12–16px) instead of `rounded.xl` (24px) — dashboards read denser with smaller radii
  - All numeric values (price, %, quantity, balance) use `font-variant-numeric: tabular-nums` with a monospace-leaning stack, right-aligned in table columns
  - Header height 56–64px (not the marketing site's spacious nav)

### Korean market convention override
`{colors.semantic-up}`/`{colors.semantic-down}` are swapped from Coinbase's own crypto convention: in AI-STOCK, **up = red** (`#cf202f`), **down = blue** (`#1d4ed8`), matching Korean securities apps (KRX convention), not Coinbase's green/red. Still text-color-only, never a background fill, per the Don't rule above.

### Always-dark surfaces
The AI advisor / market-briefing hero card on the home dashboard and AI feature shells intentionally keeps the `surface-dark` treatment (`{colors.surface-dark}` / `{colors.surface-dark-elevated}`) regardless of the rest of the page being light — this is a deliberate brand accent, not a leftover from the old all-dark theme.
